...
\end{minipage}
\begin{minipage}{0.5\textwidth}
  \includegraphics[width=\linewidth]{../catalogue/select-04-linear.png}
  \captionof{figure}{Visualisation of the decision boundary of a SVM with a linear kernel.}

  \includegraphics[width=\linewidth]{../catalogue/select-04-rbf.png}
  \captionof{figure}{Visualisation of the decision boundary of a SVM with a RBF kernel.}
  \label{fig:svm}
\end{minipage}

Figures~\ref{fig:svm} presents visualisations obtained from a notebook that performs binary classification using Support Vector Machines (SVM). The model is trained on the \texttt{make\_circles}~\footnote{REFME} dataset obtained from scikit-learn~\footnote{REFME}. The visualisations show the two classes present in the training set, and the decision boundary of the model when trained first using a linear kernel and later with a RBF kernel. Listing~\ref{lst:svm} shows the corresponding assertion that was defined below the visualisations presented in Figure~\ref{fig:svm}. The assertion checks if the model accuracy is above a threshold specified by the author of the notebook.
