...
\begin{revcomment}[2.4]
  ...
  % TODO: we explored this idea but we observed that large notebooks could be (uninteresting) eg. notebook X while smaller... link this with SS (gives similar weightable to all FMs without making assumptions about complexity or other properties)
  
  Including this stratified analysis would significantly strengthen the study's contributions.
\end{revcomment}

% TODO also add the benefits of using stratified sampling to ensure diversity in population of FMs; "while not perfect, we use SS to increase diversity of our candidates"
% TODO add example of a really large notebook with simple FM and a small one with complex FM
...
\begin{quote}
  ...
  % TODO: after introducing TF-IDF add why this is useful for us; it allows us to find not only common FMs but also specialized ones that address corner cases
...
% TODO: our goal was not to create an extensive taxonomy of FMs, but to validate if there is something worth future research; this supports the use of a time budget for the analysis
% TODO: in order to scale analysis to a generalizable population, we need automated tools; our methodology does not scale; smaller but insightful analysis; to scale we need to trade-off between deep insights and generalizability
