...
% we use feedback in notebooks to make important decisions: what data pre-processing techniques to use, create new features to boost performance of a model, pick particular hyper-parameters. but these decisions only reflect the conditions when the model was created; things inevitably change: data, people, system; if we don't document these decisions then it can lead to technical debt; longer time to debug issues; which can cost companies resource (time & cost) but also imagine if the model is being used in safety critical systems: can lead to loss of human lives as well

% in software engineering, tests are used to document expectations of how the code should function. Failing tests indicate that our expectations no longer satisfied. This is why we also look for assertions written in notebooks and analyse their use.
