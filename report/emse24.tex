...
% NOTE use background section from pimentel2019large-scale

% describe the structure of jupyter notebooks (anatomy: cells, outputs); no need to go into too much technical detail regarding the internal JSON structure, just mention that the data is represented in a standardised structure internally
...
% the role of early adoption of feedback when developing software, agile
% similar concept exists in ML: crisp-dm, mldl, sculley, amershi, sambasivan
% feedback from one part of the pipeline feeds into another, 

\subsection{Computational Notebooks and Software Engineering}\label{sec:notebooks}

% start with a very quick review of the workflow: REPL style of development; break code into smaller chunks; faster feedack; iterative style of development
% link to the existing work on notebooks from chi: design, benefits and challenges
% then go into the SE side of software written in notebooks: code quality
% adoption of notebooks by ML community

% start with a quick introduction to the anatomy of a computational notebook: code is broken down into smaller chunks represented as cells, cells can contain code (which are executable and immediately display the result of the execution), or markdown cells used for the purpose of documenting or creating a narrative for collaborative work.

% the move on to the benefits of this style of coding: the ML pipeline contains several components, which can be broken down into smaller, more manageable chunks. These can be individually developed incorporating the rapid feedback provided from the REPL style of coding.

% then summarise the pain-points of working in notebooks: use chattopadhyay2020whats
