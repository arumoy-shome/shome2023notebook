...
Section~\ref{sec:rq1} establishes what feedback statements practitioners author
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.6\textwidth}
    ...
    \caption{Type distribution within each intent, normalized within intent.}
    \label{fig:distribution-type}
  \end{subfigure}
  \caption{Distribution of feedback statements across intent and type.
...
EXP accounts for 85\% ($n=692$) of the corpus
and VAL accounts for the remaining 15\% ($n=123$).
...
This asymmetry is consistent with the contrasting purposes of the two platforms.
...
\textbf{Of the 123 VAL statements, and only four occur in Kaggle notebooks.}
...
We organize the description by intent and by type frequency.
A more detailed version of the taxonomy can be found in our replication package~\cite{replication}.
...
that the framework does not check at runtime (Listing~\ref{lst:4}),
...
and how the choice of intent and its type varies by stage.
...
stratified by source to control for the platform-level effect established in RQ1.
Finally, we examine type by stage on the GitHub subset only,
because Kaggle's heavy concentration in data preparation
and its small VAL count
prevent meaningful comparisons.
Note that the corpus contains 2 statements (0.25\%) where the stage was unclear (UNC),
and are excluded from the analysis,
leaving $n=813$ statements across six stages.
...
A $\chi^2$ test indicates a highly significant and large association
between the ML pipeline stage and the notebook source
...
This pattern reflects the competition format,
...
since RQ1 established that it has an effect on both variables.
For Kaggle statements ($n=433$),
Fisher's exact test reveals a statistically significant association
($p=0.009$, Cram\'er's $V=0.364$),
but the result rests on only 4 VAL statements,
which makes the effect size estimate unstable.

We therefore restrict further analysis to GitHub statements ($n=380$),
where a $\chi^2$ test reveals a statistically significant association
...
\textbf{Practitioners validate when the code defines reusable structure,}
...
\begin{figure}
  ...
  \end{center}
  \caption{Type distribution by stage on GitHub statements.
  Within EXP, COMPUTE peaks in TRAIN and EVAL, STATS in PREP and DATA,
  and INSPECT dominates every stage.
  Within VAL, SHAPE is the modal type in four of six stages,
...
\begin{table}
  \centering
  \caption{Mapping of VAL types to crash types reported by Wang et al.~\cite{wang2025why}.
  Each row identifies the crash types that the corresponding VAL type is designed to pre-empt.}
...
bbefore the dependent operation runs
surface the missing dependency as an explicit assertion failure
rather than a cryptic downstream error.

% TODO: revisit this part, is there another way to organize this section?
\textbf{VAL-TYPE} pre-empts the API-misuse subset of type-related crashes.
Wang et al.\ report that 42\% of Type Error and 44\% of Attribute Error crashes
share API misuse as a root cause.
VAL-TYPE statements enforce the type contract the API expects.
...
because they are not preventable through code-level assertions.
...
\textbf{The two studies converge on data preparation
...
% TODO: the automated too angle is possibly unnecessary
%
% This observed pattern
% reveals a class of latent assertions
% that are already present in notebooks.
...
Our results from RQ1 (Section~\ref{sec:rq1})
...
% TODO: a bit far-fetched, but okay
Second,
...
We document the mapping rule explicitly in our replication package~\cite{replication}
...
to characterize feedback practice in machine learning Jupyter notebooks.
The resulting taxonomy distinguishes exploratory feedback from formal validation
and shows that feedback in ML notebooks is predominantly visual.
Mapping our taxonomy onto an existing taxonomy of notebook crashes
reveals that the two converge on data preparation,
while ours additionally captures defensive practices
that prevent silent failures invisible to crash analysis.
