...
We apply Pearson chi-squared ($\chi^2$) test to determine if there is a significant difference
...
For distributions with a small sample size ($<5$),
we apply Barnard's exact test,
...
We apply $\chi^2$ test
to examine whether intent is associated with the source of the notebook.
All expected cell frequencies exceed 5(min = 57.7),
and satisfy the validity conditions of the test.
The test reveals a significant association between intent and source ($\chi^2(1) = 144.72$, $p < 0.001$, Cramer's $V = 0.421$).
From Figure~\ref{fig:distribution-intent} we see that
GH notebooks contain proportionally more formal assertions (31.2\%),
while KG notebooks are overwhelmingly exploratory and only contain four assertions (0.9\%).
...
\begin{figure}
  ...
  \end{center}
  \caption{Distribution of the EXP sub-types across GH and KG.}\label{fig:distribution-intent-exp}
\end{figure}

We restrict the analysis to \emph{EXP} coded statements ($N=692$)
and apply $\chi^2$ test to examine whether EXP subtype (Type) is independent of notebook source.
All expected cell frequencies exceed 5 (min = 29.64).
The test reveals a statistically significant but weak association ($\chi^2(3) = 10.77$, $p=0.013$, Cramer's $V=0.125$).
...
\begin{figure}
  ...
  \end{center}
  \caption{Distribution of the VAL sub-types across GH and KG.}\label{fig:distribution-intent-val}
\end{figure}
...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.95\textwidth]{distribution-stage.pdf}
  \end{center}
  \caption{Distribution of the stage of the validation statements across GH and KG.}\label{fig:distribution-stage}
\end{figure}
...
% We apply $\chi^2$ test
% to examine whether intent is associated with the ML development stage.
% However in the analysis above we observe that intent and stage are both associated with the source.
% Therefore, in this test source is a confounding variable since any observed association between intent and stage
% could be partially or entirely explained by their shared dependence on source.

% We therefore conduct the test separately within GH and KG.
% Within GH the association is significant ($\chi^2(5) = 20.35$, p=0.001, Cramer's $V=0.231$).
% We observe that VAL concentrates disproportionately at MODEL (GH-VAL: 21\%, GH-EXP: 6\%).

% The KG stratum is not formally testable since there are only four VAL statements,
% which makes several expected frequencies to be less than 1.
% This renders both $\chi^2$ and Fisher's exact test inconclusive.
