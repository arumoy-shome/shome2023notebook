...
collected from GitHub ($n=383$, 46\%) and Kaggle ($n=433$, 54\%).
...
\textbf{Intent varies sharply by platform}.
...
\textbf{Within-intent subcode distributions.}
...
The rank order is consistent with a \emph{notebook-as-scratchpad} model of feedback.
...
This indicates that Kaggle contributors rarely use structural validation.
...
\textbf{Stage distribution and platform asymmetry}.
...
These statements are excluded which leaves $N=813$ statements across six stages.
The test indicates a highly significant and large association
($\chi^2(5) = 158.04$, $p<0.001$, Cram\'er's $V=0.441$),
and Figure~\ref{fig:distribution-stage} shows the distribution.
...
\textbf{Intent varies with stage on GitHub.}
We examine if the intent of statements
is associated with the stage in which they occur.
...
and Figure~\ref{fig:distribution-stage-intent} shows that the VAL share varies by stage.
...
\textbf{Within each intent, subcode is largely stage-agnostic.}
...
and compare the stage distribution of feedback statements
...
\textbf{Mapping VAL subcodes to crash types.}
...
\textbf{Convergence on data preparation.}
...
In this section,
we interpret our key findings,
connect them to prior work,
and discuss implications for researchers, practitioners and tool designers.

\textbf{Implicit validation through last-cell expressions}.
...
% TODO: incorporate Listing 480 into this dicussion point
