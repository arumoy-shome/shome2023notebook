...
and offers extensive tools for project management
...
which lets us capture validation behaviour beyond the Python standard library.
For last statements, we restrict our analysis to cells that produced output
...
to answer ``What the developer is trying to learn or confirm?'' with the statement (RQ2),
and ``Where in the development lifecycle?'' is the statement defined (RQ3).
...
After two additional iterations, no new codes are added to the codebook.
The stability of the codebook is then verified with a fresh set of 50 statements,
and a Cohen's kappa score of 0.8 is observed, which indicates high agreement.
Following this the first author analyzes and codes all remaining statements.

While conducting the manual analysis,
we observe 91 feedback statements that fall outside the scope of this study,
grouped into 6 exclusion categories.
We exclude exploratory statements
used to probe an API
or inspect how an imported function behaves (EC:EXP);
statements from scientific simulations
that do not involve training an ML model (EC:NOML);
misused code cells
that contain block-quoted documentation
or commented-out code with stale output (EC:ERROR);
statements with no functional intent relevant to this study,
such as \lstinline{plt.ylabel('Win %')} (EC:NOINT);
statements not authored in English (EC:ENG);
and assertions found in academic notebooks used for automatic grading (EC:HW).
...
\subsection{RQ1: Prevalence and Variation of Feedback Statements}
\label{sec:rq1}
