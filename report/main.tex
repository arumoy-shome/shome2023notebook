...
and 0.94 for the stage dimension is observed,
...
\begin{table}
  ...
  derived through open coding of 816 feedback statements.}\label{tab:taxonomy}
...
Exploratory statements (EXP) accounts for 85\% ($n=693$) of the corpus
and validation statements (VAL) accounts for the remaining 15\% ($n=123$).
...
confirming that the two platforms GH and KG
...
\begin{figure}
  ...
  is highest in the MODEL, TRAIN, and FEAT stages.}
\end{figure}
...
Note that the corpus contains 2 GitHub validation statements (0.25\%)
where the stage was unclear (UNC),
and are excluded from the analysis,
leaving $n=814$ statements across six stages.
...
concentrate predominantly on the PREP stage (58\%),
...
We therefore restrict further analysis to GitHub statements ($n=381$),
...
% NOTE: RQ3 indicates that feedback is primarily exploratory in DATA and EVAL.
% We can link this to the data-centric ML philosophy and echo how testing the data stages is really important.
...
% TODO: the automated tool angle is possibly unnecessary
%
...
and code a sample of 816 statements
