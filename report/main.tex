...
that surface information about program execution
and use this information
to decide what to do next.
We call these \emph{feedback statements}
and study two forms.
Exploratory statements display values for visual inspection,
while validation statements enforce conditions programmatically through assertions.

\textbf{Aim.}
Prior empirical work on ML notebooks
have examined points where execution halts
by analyzing crashes and bugs.
In contrast, this study examines
what practitioners check when execution does not halt,
...
\textbf{Results}.
We find that feedback in ML notebooks is overwhelmingly exploratory,
and the two platforms host qualitatively different modes of ML work.
We further map our taxonomy to an existing crash taxonomy.
We find that both converge on data preparation
as the empirical centre of gravity in ML notebook development.
Additionally, our taxonomy captures defensive practices
that crash analysis cannot observe.

\textbf{Conclusion.}
Our findings indicate
that ML notebook feedback is predominantly visual rather than programmatic,
and that source platform should be treated
as a confounder in studies of ML developer practice.
We release the corpus of 1,092,780 feedback statements
and the codebook,
...
and finds that formal assertions account for 15\% of feedback statements
...
and code a sample of 815 statements
along their functional intent and ML pipeline stage,
to characterize the feedback practice
in machine learning Jupyter notebooks.
The resulting taxonomy distinguishes
exploratory feedback from formal validation,
and finds that feedback in ML notebooks is predominantly visual.
Further mapping with existing taxonomy of crashes in Jupyter notebooks,
shows that both taxonomies converge on data preparation
as the empirical centre of gravity in ML notebook development,
and our taxonomy additionally captures defensive practices
that prevent silent failures
that crash analysis cannot observe.
Future work should extend the analysis
to industrial ML notebooks
where production handoffs and code review
may impose stricter validation discipline,
and to longitudinal corpora
that capture how feedback practice evolves
as notebook tooling matures.
