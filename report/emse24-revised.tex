...
% NOTE: above number extracted using the following shell command: `cat data-collection.log |grep '^OUTPUT'|cut -d: -f2|sed -e 's/-asserts\.csv//' -e 's/-prints\.csv//' -e 's/-lasts\.csv//'|sort|uniq|wc -l`
\begin{figure}
	\centering
